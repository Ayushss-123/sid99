// https://github.com/mozmorris/react-ReactWeb
...
export default class ReactWeb extends Component {
  ...
  componentDidMount() {
    ...
    ReactWeb.mountedInstances.push(this);

    if (!this.state.hasUserMedia && !ReactWeb.userMediaRequested) {
  ...
  componentWillUnmount() {
    const index = ReactWeb.mountedInstances.indexOf(this);
    ReactWeb.mountedInstances.splice(index, 1);

    ReactWeb.userMediaRequested = false;
    if (ReactWeb.mountedInstances.length === 0 && this.state.hasUserMedia) {
    ...
    if (window.reader) {
      ...
      .then((results) => {
        if(!_.isEmpty(results)) {
            let str = String.fromCharCode.apply(String, results[0].BarcodeBytes);
        }
  ...
  requestUserMedia() {
    ...
    const sourceSelected = (audioConstraints, videoConstraints) => {
      ...
      navigator.mediaDevices
        ...
        .then((stream) => {
          ReactWeb.mountedInstances.forEach(instance =>
    ...
    ReactWeb.userMediaRequested = true;
  }
